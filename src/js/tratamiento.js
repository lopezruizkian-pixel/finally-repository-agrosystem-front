...
async function updateTratamientoBackend(idTratamiento, payload, idUsuarioHeaderOverride){
  try{
    ...
    const res = await fetch(`http://192.168.1.17:7002/tratamientos/${idTratamiento}`, { method: 'PUT', headers, body: JSON.stringify(payload) });
...
async function deleteTratamientoBackend(idTratamiento){
  try{
    ...
    const res = await fetch(`http://192.168.1.17:7002/tratamientos/${idTratamiento}`, { method: 'DELETE', headers });
    const text = await res.text(); let body = text? ((()=>{ try{return JSON.parse(text);}catch(e){return text;} })()):null;
...
btnGuardar.addEventListener('click', () => {
  ...
  if(editIndex !== null){
    ...
    try{
      ...
      const res = await fetch('http://192.168.1.17:7002/tratamientos', { method: 'POST', headers, body: JSON.stringify(payload) });
      const text = await res.text(); let body = text? ((()=>{ try{return JSON.parse(text);}catch(e){return text;} })()):null;
