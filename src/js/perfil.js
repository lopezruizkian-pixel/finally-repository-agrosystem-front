...
async function resolveUsuarioId(candidate) {
    ...
    try {
        ...
        const res = await fetch('http://192.168.1.17:7002/usuarios', {
            method: 'GET',
            headers: {
...
async function fetchUsuarioPerfil() {
    try {
        ...
        if (candidate && !isNaN(Number(candidate))) {
            ...
            const res = await fetch(`http://192.168.1.17:7002/usuarios/${id}`, { method: 'GET', headers });
            if (res.ok) {
        ...
        const resAll = await fetch('http://192.168.1.17:7002/usuarios', { method: 'GET', headers });
