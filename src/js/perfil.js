...
async function resolveUsuarioId(candidate) {
    ...
    try {
        ...
        const res = await fetch('http://54.243.211.195:7002/usuarios', {
            method: 'GET',
            headers: {
...
async function fetchUsuarioPerfil() {
    try {
        ...
        if (candidate && !isNaN(Number(candidate))) {
            ...
            const res = await fetch(`http://54.243.211.195:7002/usuarios/${id}`, { method: 'GET', headers });
            if (res.ok) {
        ...
        const resAll = await fetch('http://54.243.211.195:7002/usuarios', { method: 'GET', headers });
